...
def main() -> int:
    parser = argparse.ArgumentParser(description="Analyze rANS context-coded bitstream compression ratios.")
    ...
    if args.include_meta and codebook_bytes:
        totals["k_comp"] += codebook_bytes / 2.0
        totals["v_comp"] += codebook_bytes / 2.0

    csv_path = os.path.join(args.out_dir, "rans_metrics.csv")
    ...
    summary_path = os.path.join(args.out_dir, "rans_summary.md")
    with open(summary_path, "w", encoding="utf-8") as f:
        f.write("# rANS Context Summary\n\n")
        f.write(f"- Dumps analyzed: {len(rows)}\n")
        f.write(f"- Include meta bytes: {bool(args.include_meta)}\n\n")
        if args.include_meta and codebook_bytes:
            f.write(f"- Codebook bytes: {codebook_bytes}\n\n")
