...
# ============================================================================
# H2O v5 Final Acceptance Runner
...
BASE_OUT="${BASE_OUT:-exp/h2o_v5/out_final_v5_$(date +%Y%m%d_%H%M%S)}"
...
run_m3_case() {
  ...
  if ! env \
    ...
    bash exp/h2o_v5/test_v5_m3.sh > "${console_log}" 2>&1; then
    echo "FAIL: ${case_name}"
    echo "See: ${console_log}"
    tail -n 80 "${console_log}" || true
    exit 1
...
lines = []
lines.append("# H2O v5 Final Acceptance Report")
lines.append("")
