...
if [[ $# -lt 3 ]]; then
  echo "Usage: $0 <out_dir> <dump_dir> <lossless_target> [decode_baseline] [decode_drop_target] [online_chunk_seq] [online_framing_bytes] [adaptive_block_seq] [online_entry_mode] [require_runtime_decomp] [max_lossless_queue_peak] [max_lossless_fallback] [max_lossless_decomp_us]"
  echo "Example: $0 exp/h2o_v5/out_tune_v5_20260212 /home10T/ljq/MNN/exp/gear_fp16/dumps_fp16 1.3 6.72 0.05 192 12 64 chunked_grouped true 8 0 -1"
  ...
  echo "max_lossless_queue_peak/max_lossless_fallback/max_lossless_decomp_us: negative disables each gate"
  exit 1
fi
...
MAX_LOSSLESS_QUEUE_PEAK="${11:-8}"
MAX_LOSSLESS_FALLBACK="${12:-0}"
MAX_LOSSLESS_DECOMP_US="${13:--1}"
...
if [[ -d "${OUT_DIR}/logs" ]] && ls "${OUT_DIR}/logs"/*.log >/dev/null 2>&1; then
  python3 exp/h2o_v5/parse_h2o_v5_log.py \
...
python3 exp/h2o_v5/offline_lossless_fp16.py \
  --dump-dir "${DUMP_DIR}" \
  --scope front_n \
  --front-n 2 \
  --stage both \
...
run_summary() {
  ...
  python3 exp/h2o_v5/analyze_h2o_v5.py "${args[@]}"
}
