...
def main():
    ...
    parser.add_argument("--preset", required=True, help="Preset json, see exp/h2o_v5/configs/*.json")
    parser.add_argument("--out-dir", default="exp/h2o_v5/out")
    ...
    cmd = [
        sys.executable,
        str(Path(__file__).with_name("run_h2o_v5_bench.py")),
        "--llm-bench",
        args.llm_bench,
        "--base-config",
        args.base_config,
        ...
        "--kv-lossless-scope",
        str(preset.get("kv_lossless_scope", "front_n_and_h2o_kept")),
        "--kv-lossless-front-n",
        str(preset.get("kv_lossless_front_n", 2)),
        "--kv-lossless-block-tokens",
        str(preset.get("kv_lossless_block_tokens", 128)),
        "--kv-lossless-hot-recent-tokens",
        str(preset.get("kv_lossless_hot_recent_tokens", 256)),
        "--kv-lossless-hot-sink-tokens",
        str(preset.get("kv_lossless_hot_sink_tokens", 16)),
        "--kv-lossless-kept-sample-layers",
        ...
        "--kv-lossless-store-bootstrap-tokens",
        str(preset.get("kv_lossless_store_bootstrap_tokens", 32)),
        "--kv-lossless-store-grouped-step-tokens",
        str(preset.get("kv_lossless_store_grouped_step_tokens", 256)),
        ...
        str(preset.get("kv_lossless_predictors_k", "raw,delta_seq,xor_seq,pair_delta")),
        "--kv-lossless-predictors-v",
        str(preset.get("kv_lossless_predictors_v", "raw,delta_seq,xor_seq")),
        "--kv-lossless-async-threads",
        str(preset.get("kv_lossless_async_threads", 1)),
        "--kv-lossless-max-queue",
        str(preset.get("kv_lossless_max_queue", 256)),
        "--kv-lossless-decode-cache-blocks",
        str(preset.get("kv_lossless_decode_cache_blocks", 64)),
    ]
    ...
    if preset.get("kv_lossless_strict_roundtrip_check", False):
        cmd.append("--kv-lossless-strict-roundtrip-check")
