...
# ============================================================================
# H2O v4 Runtime Validation Script
# Coverage:
...
#   - v4 strict runtime decode evidence (h2o_lossless_decomp_us > 0)
...
OUT="${OUT:-exp/h2o_v4/out_runtime_v4_$(date +%Y%m%d_%H%M%S)}"
...
H2O_RECENT_TOKENS="${H2O_RECENT_TOKENS:-64}"
...
# Lossless
KV_LOSSLESS_SCOPE="${KV_LOSSLESS_SCOPE:-front_n}"
...
JSONEOF
set +e
EMPTY_OUT=$(python3 exp/h2o_v4/run_h2o_v4_bench.py \
  --llm-bench "${LLM_BENCH}" \
...
CSVEOF
GATE_OUT=$(python3 exp/h2o_v4/analyze_h2o_v4.py \
...
python3 exp/h2o_v4/run_h2o_v4_bench.py "${BENCH_ARGS[@]}" --dry-run
...
python3 exp/h2o_v4/run_h2o_v4_bench.py "${BENCH_ARGS[@]}" 2>&1 | tee "${BENCH_STDOUT_LOG}"
...
echo ""
echo "==== Step 4.2: Validating runtime decompression (v4 strict) ===="
