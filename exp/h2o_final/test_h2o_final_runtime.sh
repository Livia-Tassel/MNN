...
KV_LOSSLESS_STRICT_ROUNDTRIP_CHECK="${KV_LOSSLESS_STRICT_ROUNDTRIP_CHECK:-1}"
...
ASYNC_WAIT_HEADROOM_WARN_RATIO="${ASYNC_WAIT_HEADROOM_WARN_RATIO:-0.15}"
...
REQUIRE_KV_CONTENT_CONSISTENCY="${REQUIRE_KV_CONTENT_CONSISTENCY:--1}"
MAX_KV_INVALID_LOG_LINES="${MAX_KV_INVALID_LOG_LINES:-0}"
...
if [[ "${REQUIRE_KV_CONTENT_CONSISTENCY}" -lt 0 ]]; then
  if [[ "${DISABLE_H2O}" -ne 0 || "${DISABLE_LOSSLESS}" -ne 0 || "${KV_LOSSLESS_RUNTIME_MODE}" == "probe" ]]; then
...
echo "  ASYNC_WAIT_HEADROOM_WARN_RATIO=${ASYNC_WAIT_HEADROOM_WARN_RATIO}"
...
echo "  KV_LOSSLESS_STRICT_ROUNDTRIP_CHECK=${KV_LOSSLESS_STRICT_ROUNDTRIP_CHECK}"
echo "  REQUIRE_RUNTIME_DECOMP=${REQUIRE_RUNTIME_DECOMP}"
echo "  REQUIRE_KV_CONTENT_CONSISTENCY=${REQUIRE_KV_CONTENT_CONSISTENCY}"
echo "  MAX_KV_INVALID_LOG_LINES=${MAX_KV_INVALID_LOG_LINES}"
...
if [[ "${KV_LOSSLESS_STRICT_ROUNDTRIP_CHECK}" -ne 0 ]]; then
  BENCH_ARGS+=(--kv-lossless-strict-roundtrip-check)
fi
...
ANALYZE_ARGS=(
  ...
  --async-wait-headroom-warn-ratio "${ASYNC_WAIT_HEADROOM_WARN_RATIO}"
  ...
  --runtime-mode "${KV_LOSSLESS_RUNTIME_MODE}"
  --async-threads "${KV_LOSSLESS_ASYNC_THREADS}"
  --bench-stdout-log "${BENCH_STDOUT_LOG}"
  --logs-dir "${OUT}/logs"
  --max-kv-invalid-log-lines "${MAX_KV_INVALID_LOG_LINES}"
  --out "${OUT}/summary.md"
)
if [[ "${REQUIRE_RUNTIME_DECOMP}" -ne 0 ]]; then
  ANALYZE_ARGS+=(--require-runtime-decomp)
fi
if [[ "${REQUIRE_KV_CONTENT_CONSISTENCY}" -ne 0 ]]; then
  ANALYZE_ARGS+=(--require-kv-content-consistency)
fi
