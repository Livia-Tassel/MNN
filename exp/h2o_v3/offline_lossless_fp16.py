...
def write_markdown(path, result):
    with Path(path).open("w", encoding="utf-8") as f:
        f.write("# H2O v3 Offline Lossless Summary\n\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Offline lossless ratio eval for H2O v3 (FP16 predictive codec)."
    )
    ...
    if args.verbose:
        print(
            f"[offline_lossless_fp16 v3] entry_mode={args.entry_mode} "
